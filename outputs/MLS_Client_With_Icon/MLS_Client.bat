...
rem These values can be overridden before launching this file when the server changes.
if not defined MLS_SERVER_NAME set "MLS_SERVER_NAME=DESKTOP-LARAL6N"
if not defined MLS_SERVER_IP set "MLS_SERVER_IP=192.168.254.192"
...
rem Prefer the fixed LAN address. It is also the origin configured by the static
rem server and avoids hostname/CORS differences between Windows client PCs.
call :check_server "http://%MLS_SERVER_IP%:%MLS_SERVER_PORT%"
if not errorlevel 1 goto :connected

echo Fixed server address %MLS_SERVER_IP% was not found.
