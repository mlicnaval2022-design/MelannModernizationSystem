...
const getCollectionBreakdown = (collections, passbooks, penalties, collectorsOver, otherTransactions = []) => {
  const breakdown = {
    regular: 0,
    balance: 0,
    penalty: 0,
    ...
    other_transactions: sumAmount(otherTransactions)
  };
...
router.get('/summary', authenticateToken, async (req, res) => {
  try {
    ...
    // YTD defaults
    const ytd_beg_releases_default = ytdOverride ? ytdOverride.ytd_beg_releases : (prevDcr ? (prevDcr.ytd_beg_releases + prevDcr.total_releases) : 0);
    const ytd_beg_collections_default = ytdOverride ? ytdOverride.ytd_beg_collections : (prevDcr ? (prevDcr.ytd_beg_collections + prevDcr.total_collections) : 0);
    const ytd_beg_expenses_default = ytdOverride ? ytdOverride.ytd_beg_expenses : (prevDcr ? (prevDcr.ytd_beg_expenses + prevDcr.total_expenses) : 0);

    // Cash available and expected ending cash
    ...
    const ledger = [
      ...
      ...otherTransactions.map(c => ({ type: 'Other Transactions', ref: '—', code: '—', name: c.description || 'Unassigned', amount: c.amount, user: c.encoded_by, time: c.created_at, remarks: 'Other Transactions', dcr_id: c.dcr_id }))
