...
router.post('/scan', authenticateToken, requireRole('admin', 'manager'), async (req, res) => {
  try {
    ...
    res.json(await scanJcash({ from, to, password }));
  } catch (err) {
...
router.post('/migrate', authenticateToken, requireRole('admin', 'manager'), async (req, res) => {
  try {
    const { from, to, loan_codes, password } = req.body;
    res.json(await migrateSelectedJcash({ from, to, loanCodes: loan_codes, user: req.user, password }));
  } catch (err) {
    sendRouteError(res, err);
  }
});
