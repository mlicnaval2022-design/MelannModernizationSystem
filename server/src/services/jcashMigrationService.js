...
const EXCLUDED_STATUS = new Set(['fully paid', 'fullypaid', 'full paid', 'fullpaid', 'paid', 'reverse', 'reversed', 'reversing']);
...
function readAccessRows({ from, to, source = process.env.JCASH_MDB_PATH || DEFAULT_SOURCE, password }) {
  validateDateRange(from, to);
  const accessPassword = getAccessPassword(password);
  const fromDate = accessDateLiteral(from);
  const toDate = accessDateLiteral(to);
...
async function scanJcash({ from, to, password }) {
  const snapshot = readAccessRows({ from, to, password });
  ...
  const loans = snapshot.loans
    ...
    .filter(loan => isGoodStatus(loan.source_loan_status) && isGoodStatus(loan.source_row_status || loan.source_loan_status))
    .filter(loan => !isExcludedStatus(loan.source_loan_status) && !isExcludedStatus(loan.source_row_status))
    .filter(loan => loan.date_released >= from && loan.date_released <= to);
...
async function migrateSelectedJcash({ from, to, loanCodes = [], user, password }) {
  if (!Array.isArray(loanCodes) || loanCodes.length === 0) {
    const err = new Error('Select at least one loan to migrate.');
    err.statusCode = 400;
    throw err;
  }
  const scan = await scanJcash({ from, to, password });
  ...
  try {
    ...
    await dbRun(
      ...
      [user?.id || null, user?.username || 'system', 'MIGRATE', 'JCASH MIGRATION', `Migrated ${stats.loans_inserted} new and ${stats.loans_updated} existing Good loans from ${from} to ${to}. Payments inserted: ${stats.payments_inserted}.`]
    );
    await dbRun('COMMIT');
...
module.exports = { scanJcash, migrateSelectedJcash, validateDateRange };
