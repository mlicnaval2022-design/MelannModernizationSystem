...
async function initializeDatabase() {
  const schema = `
    ...
    CREATE TABLE IF NOT EXISTS tblCollectorOption (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      option_type TEXT NOT NULL CHECK(option_type IN ('assigned_area', 'supervisor')),
      option_name TEXT NOT NULL COLLATE NOCASE,
      is_active INTEGER NOT NULL DEFAULT 1,
      created_at TEXT DEFAULT (datetime('now')),
      updated_at TEXT DEFAULT (datetime('now')),
      UNIQUE(option_type, option_name)
    );
    CREATE INDEX IF NOT EXISTS idx_collector_option_type_active
      ON tblCollectorOption(option_type, is_active, option_name);
  ...
  const defaultCollectorOptions = {
    assigned_area: ['ORMOC', 'KANANGA', 'CARIGARA', 'ISABEL', 'BAYBAY/HILONGOS/BATO', 'SAN ISIDRO'],
    supervisor: ['Lahoylahoy, Wilfredo', 'Langres, Ramil', 'Omega, Raymund'],
  };
  for (const [optionType, optionNames] of Object.entries(defaultCollectorOptions)) {
    for (const optionName of optionNames) {
      await dbRun(
        `INSERT OR IGNORE INTO tblCollectorOption (option_type, option_name) VALUES (?, ?)`,
        [optionType, optionName]
      );
    }
  }
  await dbRun(`
    INSERT OR IGNORE INTO tblCollectorOption (option_type, option_name)
    SELECT 'assigned_area', TRIM(assigned_to)
    FROM tblCollector
    WHERE TRIM(COALESCE(assigned_to, '')) <> ''
  `);
  await dbRun(`
    INSERT OR IGNORE INTO tblCollectorOption (option_type, option_name)
    SELECT 'supervisor', TRIM(supervisor)
    FROM tblCollector
    WHERE TRIM(COALESCE(supervisor, '')) <> ''
  `);
  ...
  const extCols = ['sitio', 'purok', 'brgy', 'city', 'gender', 'secondary_contact', 'email', 'income_per_month', 'expenses_per_month', 'loan_purpose', 'collateral', 'id_type', 'id_number', 'id_issue_date', 'id_expiry_date', 'id_issued_by', 'fb_account', 'nationality', 'educational_background', 'occupational_status', 'customer_classification', 'risk_category', 'cic_verification', 'province', 'zip_code', 'length_of_stay', 'previous_address', 'messenger_account', 'preferred_contact_method', 'preferred_contact_time_from', 'preferred_contact_time_to', 'contact_notes', 'business_type', 'business_name', 'business_employees', 'permit_date_issued', 'permit_place_issued', 'permit_no', 'id_place_of_issue', 'tin_number', 'sss_number', 'id_notes', 'photo_id_front', 'photo_id_back', 'photo_business_proof', 'photo_client', 'death_certificate_image'];
