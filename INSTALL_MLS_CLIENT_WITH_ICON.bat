...
set "MLS_CERT_SOURCE=%MLS_SOURCE_DIR%MLS_SERVER_CERT.cer"
...
set "MLS_CERT_TARGET=%MLS_INSTALL_DIR%\MLS_SERVER_CERT.cer"
...
if not exist "%MLS_CERT_SOURCE%" (
  echo ERROR: MLS_SERVER_CERT.cer is missing.
  echo Copy the certificate generated by the branch server into this installer folder.
  echo.
  pause
  exit /b 1
)
...
copy /Y "%MLS_CERT_SOURCE%" "%MLS_CERT_TARGET%" >nul
if errorlevel 1 goto :install_error

echo   Installing the branch HTTPS trust certificate...
certutil.exe -user -f -addstore Root "%MLS_CERT_TARGET%" >nul
if errorlevel 1 goto :certificate_error
...
:certificate_error
echo ERROR: Windows could not trust the branch HTTPS certificate.
echo The client files were copied, but secure server access will not work yet.
echo.
pause
exit /b 1
