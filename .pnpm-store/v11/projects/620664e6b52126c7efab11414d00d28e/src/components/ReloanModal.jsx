...
export default function ReloanModal({ isOpen, onClose, customerId, customer, loanType = 'RELOAN', onReloanSubmitted, onViewSoa }) {
  ...
  const outstandingLoan = (account?.loans || []).find(l => {
    const loanStatus = String(l.status || '').toLowerCase();
    return Number(l.balance || 0) > 0 && !['reversed', 'rejected', 'cancelled', 'canceled'].includes(loanStatus);
  });
  const status = String(account?.status || activeCustomer?.status || '').toUpperCase();
  const isFullyPaid = ['FULLY PAID', 'FULLPAID'].includes(status);
  ...
  const eligibility = useMemo(() => {
    ...
    if (type === 'NEW' && outstandingLoan) return { ok: false, message: 'This client already has an active loan and cannot be processed as NEW.' };
    if (type === 'RELOAN' && outstandingLoan) return { ok: false, message: 'This client has an outstanding balance and must be fully paid before processing a RELOAN.' };
    if (type === 'RELOAN' && !isFullyPaid) return { ok: false, message: 'This client must be FULLY PAID before processing a RELOAN.' };
  ...
  }, [activeCustomerId, classifiedForReloan, form.loanType, isFullyPaid, latestLoan, outstandingLoan, status]);
      ...
      <div className="reloan-modal loan-entry-modal">
        ...
        <div className="reloan-footer">
          ...
          {!preview ? (
            ...
            <button type="button" className="reloan-primary" onClick={submit} disabled={submitting || reloanBlocked}>{submitting ? 'Saving...' : 'Confirm and Save'}</button>
          )}
        </div>
      </div>
    </div>
