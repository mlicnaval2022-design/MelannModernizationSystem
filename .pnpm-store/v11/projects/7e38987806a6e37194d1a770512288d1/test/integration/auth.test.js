...
test('reloan is rejected while a client still has an outstanding balance', async () => {
  const login = await fetch(`${baseUrl}/api/auth/login`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ username: 'admin', password: 'admin123' }),
  ...
  const reloanCount = await dbGet(`SELECT COUNT(*) AS count FROM tblLoan WHERE customer_id = ? AND loan_type = 'Reloan'`, [customer.lastID]);
  const oldLoanPayment = await dbGet(`SELECT id FROM tblPayment WHERE loan_id = ?`, [sourceLoan.lastID]);

  assert.equal(response.status, 400);
  assert.match(body.error, /outstanding balance.*fully paid/i);
  assert.deepEqual(oldLoan, { balance: 500, total_paid: 500, status: 'active' });
  assert.equal(reloanCount.count, 0);
  assert.equal(oldLoanPayment, undefined);
});
