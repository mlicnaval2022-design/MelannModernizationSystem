...
router.get('/:id/reloan-eval', authenticateToken, async (req, res) => {
  try {
    ...
    const outstandingLoans = await dbGet(`
    ...
    const is_fully_paid = ['FULLY PAID', 'FULLPAID'].includes(customerStatus);
    const no_outstanding_balance = !outstandingLoans || !outstandingLoans.total_balance || outstandingLoans.total_balance <= 0;
    const is_good_standing = is_fully_paid && no_outstanding_balance;
    ...
    res.json({
      ...
      active_balance: outstandingLoans ? outstandingLoans.total_balance : 0,
...
router.post('/:id/reloan', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const outstandingLoan = await dbGet(
    ...
    if (normalizedLoanType === 'NEW' && outstandingLoan) {
    ...
    if (normalizedLoanType === 'RELOAN') {
      ...
      if (outstandingLoan) {
        const err = new Error(`This client cannot be processed as RELOAN until the outstanding balance of ${Number(outstandingLoan.balance || 0).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} is fully paid.`);
        err.statusCode = 400;
        throw err;
      }
      if (!['FULLY PAID', 'FULLPAID'].includes(customerStatus)) {
        const err = new Error('This client must be FULLY PAID before processing a RELOAN.');
        err.statusCode = 400;
        throw err;
      }
    }
