...
import { CheckSquare, Database, Download, FileText, RefreshCw, Search, Square, Trash2, X } from 'lucide-react'
...
export default function JcashMigration() {
  ...
  const [scanCriteria, setScanCriteria] = useState(null)
  ...
  const scan = async () => {
    ...
    try {
      ...
      setScanCriteria({ mode: 'date', from, to })
  ...
  const searchLoan = async (loanIdOverride = null) => {
    const requestedLoanId = String(loanIdOverride ?? loanId).trim()
    if (!/^\d+$/.test(requestedLoanId)) {
      setError('Enter a valid numeric Loan ID.')
      setNotice('')
      return
    }
    setError('')
    ...
    setScanMode('loan')
    try {
      const { data } = await API.post('/jcash-migration/scan', { loan_id: requestedLoanId, password })
      const resultRows = data.rows || []
      setRows(resultRows)
      setSummary(data.summary || null)
      setProvider(data.provider || '')
      setSource(data.source || '')
      setSelected(new Set(resultRows.map(row => row.loan.loan_code)))
      setScanCriteria({ mode: 'loan', loanId: requestedLoanId })
      setNotice(resultRows.length
        ? `Loan ID ${requestedLoanId} found. Review the client information and payments before migration.`
        : `Loan ID ${requestedLoanId} was not found or is not eligible for migration.`)
    } catch (err) {
      setRows([])
      setSummary(null)
      setSelected(new Set())
      setScanCriteria(null)
      setError(err.response?.data?.error || 'Failed to search jcashdb.mdb by Loan ID.')
    } finally {
      setScanning(false)
      setScanMode(null)
    }
  ...
  const refreshScan = () => {
    if (scanCriteria?.mode === 'loan') searchLoan(scanCriteria.loanId)
    else scan()
  }
  ...
  const migrate = async () => {
    ...
    setMigrating(true)
    try {
      const criteria = scanCriteria?.mode === 'loan'
        ? { loan_id: scanCriteria.loanId }
        : { from: scanCriteria?.from || from, to: scanCriteria?.to || to }
    ...
    <div className="jcash-page">
      <div className="jcash-hero">
        <div>
          ...
          <p>Scan eligible loans by Date Release or search one client by Loan ID, then review the read-only preview before migration.</p>
        </div>
      ...
      <div className="jcash-single-search">
        <div className="jcash-single-search-copy">
          <div className="jcash-single-search-icon"><Search size={18} /></div>
          <div>
            <div className="jcash-single-search-title">Migrate One Loan</div>
            <div className="jcash-single-search-subtitle">Search by Loan ID only. Client information and eligible payments are included in the read-only preview.</div>
          </div>
        </div>
        <div className="form-group jcash-loan-id-field">
          ...
          <input
            ...
            placeholder="Example: 30997"
          />
        </div>
        <button className="btn btn-primary jcash-loan-search-btn" onClick={() => searchLoan()} disabled={scanning || migrating || !loanId.trim()}>
          <Search size={15} /> {scanMode === 'loan' ? 'Searching...' : 'Search Loan ID'}
        </button>
      </div>
      ...
      <div className="jcash-toolbar">
        ...
        <button className="btn btn-primary jcash-scan-btn" onClick={scan} disabled={scanning || migrating}>
          <Database size={15} /> {scanMode === 'date' ? 'Scanning...' : 'Scan JCash Database'}
        </button>
      ...
      <div className="card jcash-preview-card">
        <div className="card-header jcash-preview-header">
          <div>
            ...
            <div className="card-subtitle">{provider ? `Read-only provider: ${provider}` : 'Select a date range or search by Loan ID.'}{source ? ` Source: ${source}` : ''}</div>
          </div>
          <button className="btn btn-secondary btn-sm" onClick={refreshScan} disabled={scanning || migrating || !rows.length}>
